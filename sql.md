...
CREATE TABLE public.offers (
  ...
  message text,
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'accepted'::text, 'rejected'::text, 'cancelled'::text, 'withdrawn'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.reviews (
  ...
  seller_response_created_at timestamp with time zone,
  seller_response_updated_at timestamp with time zone,
